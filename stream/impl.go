...
//
//	          head filter   map_helper   for-each
//	          +--+    +---+    +--+
//	   nil <- |  | <- |   | <- |  | <- terminalStage
//	          +--+    +---+    +--+
//
...
func (s *stream) Group(f types.Function) types.Map {
	return s.ReduceBy(func(count int64) types.R {
		return make(types.Map)
	}, func(groupMap types.R, value types.T) types.R {
		group := groupMap.(types.Map)
		key := f(value)
		val, isExist := group[key]
		if !isExist {
